function [f_estimate ,error] = int_estimate_SIR(f ,f_int ,delta_f ,SIR_dB ,N ,window_type ,estimate_type)
%函数描述：[估计频率，误差] = int_estimate(信号频率 ,干扰频率，频率分辨率 ,dB信干比 ,FFT点数 ,噪声类型，窗函数类型 ,估计器类型)
    for i = 1:length(SIR_dB)
        ...
        SIR = 10^(SIR_dB(i)/10);    %将dB信干比转换为线性信干比
        I = Eb/SIR;                 %计算干扰功率
        a_int = sqrt(2*I);          %计算干扰幅度
        signal_int = cos(2*pi*f*t) + a_int*cos(2*pi*f_int*t);   %信号叠加干扰
       ...
       disp(['进度:',num2str(i/length(SIR_dB)*100),'%']);   %显示进度百分比
    end
